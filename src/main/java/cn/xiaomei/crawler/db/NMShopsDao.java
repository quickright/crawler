 ...
 */
public class NMShopsDao {


    public static void main(String[] args) {
        NMShopsDao shopsDao = new NMShopsDao();
        try {
            //j7aUZgjsgQfVWK3MZXSCEWT6
            //qIVNZ35pYjzmvHmF80PE6lGt
            shopsDao.updatePOI(Integer.valueOf(args[0]),args[1]);
        } catch (SQLException e) {
            e.printStackTrace();
        }
//        String cityid=args[0];
        /*File dir=new File("/Users/chunli/Downloads/");
        List<String> mysqid= null;
        try {
            mysqid = FileUtils.readLines(new File("/Users/chunli/Downloads/ids.csv"));
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        for(File file:files) {
            shopsDao.listShop(file,mysqid);
        }*/
    }
    //  mc.close();


    public List<Json> listShop(File file,List<String> mysqid) {
        ...
        try {
            ...
            String [] docs;
            int i=0;
            while ((docs = reader.readNext()) != null) {
                // if(nextLine.length>6&&nextLine.toString().contains("5021259")) {
                // use comma as separator
                //    String[] country = line.split(cvsSplitBy);
               /* if(nextLine[5].contains("5021259"))
                    System.out.println(i+"," +nextLine[4] + nextLine[5] + "etc...");
*/
                //}
      ...
      //  for(String br:brs) {
           ...
           // try {
            ...
            //firmName,brandName,categoryName,cityname,address,firmId,phoneNum
           // String[] docs = br.split(",");
            String shop_id = docs.length>5?docs[5]:"";

            if (mysqid.indexOf(shop_id)<0&&docs[2].indexOf("categoryName")<0) {
                Shop shop = new Shop();
                if (shop_id != null && StringUtils.isNotBlank(shop_id.toString())) {
                    shop.setId_dp(shop_id.toString());
                }
                String firmName = docs.length>0?docs[0]:"";
                if (firmName != null && StringUtils.isNotBlank(firmName.toString())) {
                    shop.setTitle(firmName);
                } else {
                    shop.setTitle("");
                }
                String brandName = docs.length>1?docs[1]:"";
                if (brandName == null || StringUtils.isBlank(brandName.toString())) {
                    shop.setAlias(shop.getTitle());
                } else {
                    String tmp = shop.getTitle();
                    shop.setTitle(brandName);
                    shop.setAlias(tmp);
                }
                String categoryName = docs.length>2?docs[2]:"";
                String[] categorys = categoryName != null ? categoryName.split("-") : null;
                if (categorys != null) {
                ...
                String cityname = docs.length>3?docs[3]:"";
                ...
                String address = docs.length>4?docs[4]:"";
                ...
                String phone = docs.length>6?docs[6]:"";
    ...
    private void updatePOI(int idx,String key) throws SQLException {
        Connection conn=null;
        ...
        try {
            ...
            String sql ="select merchant_id,from_merchant_id,logo,city,id_dp,address from x520_xiaomei_merchant_main_nuomi_c where length(lat)<2 and from_merchant_id!=1 and merchant_id%10= "+idx;
            ...
            while (rs.next()) {
                ...
                String categoryName=rs.getString("logo");
                String[] categorys = categoryName != null ? categoryName.split("-") : null;
                if (categorys != null) {
                    shop.setComments(categoryName.replaceAll(" ...", ""));
                    String big_cate_id = category.get(categorys[0]);
                    shop.setIndustry_id(big_cate_id);
                } else {
                    shop.setIndustry_id("0");
                ...
                if (StringUtils.isNotBlank(city)) {
                    String cname = city.replaceAll("直销站", "");
                    cname = cname.substring(0, cname.length() - 1);
                    String pcid = pc.get(cname);
                ...
                String addressEncoder = null;
                try {
                    addressEncoder = URLEncoder.encode( rs.getString("address"), "utf-8");
                } catch (UnsupportedEncodingException e) {
                    e.printStackTrace();
                }
                String url = "http://api.map.baidu.com/geocoder/v2/?address=" + addressEncoder + "&output=json&ak="+key;//qIVNZ35pYjzmvHmF80PE6lGt";
                String res = HttpClientUtils.response(url);
                //System.out.println(res);
                if (res != null && StringUtils.isNotBlank(res) && res.indexOf("\"status\":0") > 0) {
                    JSONObject json = JsonUtils.getPathObject(res, "result", "location");
                ...
                long merchant_id = rs.getLong("merchant_id");
                sql = "update x520_xiaomei_merchant_main_nuomi_c set lat=?,lng=?,comments=?,add_time=?,industry_id=?" +
                        ",province_id=?,city=?,city_id=?,from_merchant_id=1" +
                        " where merchant_id=?;";

                stmt = conn.prepareStatement(sql);
                stmt.setString(1, shop.getLat());
                stmt.setString(2, shop.getLng());
                stmt.setString(3, shop.getComments());
                stmt.setString(4, Constant.getCurrentDateTime());
