...
public class ImportNuomiSH {
    public static void main(String [] args){
        ...
        dbc = mongoClient.getDatabase("crawler").getCollection("nuomi");
        File [] citys=dir.listFiles();
        for(File city:citys) {
            String cityid=city.getName();
            String cityname=stations.get(cityid);
            System.out.println(cityid+","+cityname);
            for (File jsonf : city.listFiles(new FilenameFilter() {
                ...
                try {
                    ...
                    if(JsonUtils.isValid(jsonc)) {
                        JSONArray ja = JsonUtils.getPathArray(jsonc, "res", "data");
                        for (int idx = 0; idx < ja.length(); idx++) {
                            Document dbObject = Document.parse(ja.getJSONObject(idx).toString());
                            dbObject.append("cityid", cityid);
                            dbObject.append("cityname", cityname.replaceAll("代理站", ""));
    ...
    private static   Map<String, String> stations = new HashMap<String, String>();
    static {
